...
sns.scatterplot(data=filtered_data, x="patient_age", y="feature_size", alpha=0.4, color='grey', edgecolors='none', linewidth=0)
plt.scatter(current_patient["patient_age"], current_patient["feature_size"], color='red')

# Calculate the median trendline
median_values = filtered_data.groupby("patient_age").median()["feature_size"].reset_index()
window_size = 10
median_values["smoothed_median"] = median_values["feature_size"].rolling(window=window_size, center=True, min_periods=1).median()
...
for idx, value in enumerate(current_patient_percentiles):
    if current_patient["feature_size"] <= value:
...
y2 = y1-0.18
arrow_start = (x2, y2)
arrow_end = (current_patient["patient_age"], current_patient["feature_size"])
plt.annotate("",
             xy=arrow_end,
             xycoords='data',
             xytext=arrow_start,
