...
def GetNextNumber(strText):
    i = 0
    nDotCount = 0
    bSymbolFound = False
    bDigitFound = False
    bDecimalFound = False
    sym = SubStrType.SS_NONE

    # allow negatives
    # don't allow a space in the middle of a number
    # don't allow two decimal symbols
    while (i < len(strText)) and (bSymbolFound == False):
        if (isdigit(strText[i])):
            i = i+1
            bDigitFound = True
        elif (strText[i] == ' ') and (bDigitFound == False):
            i = i + 1
        elif (strText[i] == '.') and (bDecimalFound == False):
            i = i + 1
            bDecimalFound = True

    if (bDigitFound == True):
        # digit spans from 0 to i-1
        strNum = strText[0:i-1]
        num = float(strNum)

        ## return substring
    return sym

def GetNextSymbol(strText):
    i = 0
    bSymbolFound = False
    sym = SubStrType.SS_NONE
    ...
    while (i < len(strText)) and (bSymbolFound == False):
        if (strText[i] == ','):
            sym = SubStrType.SS_COMMA
            bSumbolFound = True
        elif (strText[i] == '-'):
            sym = SubStrType.SS_DASH
            bSumbolFound = True
        else:
            i = i+1
...
# bAllowRange means allow a range of values, eg. 1-5
# bAllowSeries means allow a series of values, eg 1, 2, 5, 7
# nMaxValue is used where the range is open ended, eg. 3- (which means 3-nMaxValue)
def ParseInputInteger(bAllowRange, bAllowSeries, strMessage, nMaxValue):
    bValidResponse = False

    while (bValidResponse == False):
        # current_ss_type = SubStrType.SS_NUM

        uin = input(strMessage)
        start_pos = 0
        sym = GetNextSymbol(uin)

        if (sym == SubStrType.SS_DASH): ## we have a range value
            subnumlist, uin = GetNextStrRange(uin)
            numlist.add(subnumlist)
        elif (sym == SubStrType.SS_COMMA): ## we have a comma separated value
            next_num, uin = GetNextNumber(uin)
            numlist.add(next_num)
        else:   ## we don't have - or , so it should be a number.  white space should have been trimmed
            GetNextNumber(uin)
...
def SetLowHighFilterValues(bAlwaysUseRecommended, bNeverUseRecommended):
    extra_filter = ""
    ...
    if (bAlwaysUseRecommended == False) and (bNeverUseRecommended == False):
        user_filter_string = input("Enter low, high filter values to use? eg. " + str(guess_low_filter) + ", " + str(
            guess_high_filter) + " (blank is no filtering, (r) uses these recommended values once, (a) uses these recommended values always, (n) never uses these recommended values)? ")

        if (user_filter_string == 'a'): # we want to use filters for ALL subsequent processings
            bAlwaysUseRecommended = True

        if (user_filter_string == 'n'): # we want to use filters for ALL subsequent processings
            bNeverUseRecommended = True

    if (bAlwaysUseRecommended == True):
        user_filter_string = 'r'
    elif (bNeverUseRecommended == True):
        user_filter_string = ''
    ...
    return bAlwaysUseRecommended, bNeverUseRecommended, extra_filter, mid_point;
...
def GetColumns():
    ...
    print();
    uin1 = input("Which column numbers to use for y-axis of the graph, eg. 10, 22, 35? OR r: 1, 3, 4, 5 to calculate ratio of 1, 3 and 4,5 ")
    if ("r:" in uin1):
        # we have to calculate a ratio.  remove the r: and calculate the two columns
        temp = uin1.replace("r:", "")
        my_columns = [int(n) for n in temp.split(", ")]
        bCalculateRatios = True
    else:
        my_columns = [int(n) for n in uin1.split(", ")] #uin1.split(", ")

    return bCalculateRatios, len(df.columns), my_columns
...
bCalculateRatios, nWorkingColumn, my_columns = GetColumns()
sex, chart_both_sexes = SelectSex()

# if we are calculating ratios, add one column, once
if (bCalculateRatios):
    df.insert(nWorkingColumn, "Working", 0)

    # temp - move this code into the "continue here" below
    df.columns.values[nWorkingColumn] = "Hello"
    nRowCount = len(df.index)

    for i in range(len(df.index)):    # start at 1 to avoid header at 0
        if (i == 0):
            continue

        df.loc[i, nWorkingColumn] = f"=AF{i}/BT{i}"

        # q: how do I get column identifier (eg. AA, AZ) from number

    # test line
    print(df[[nWorkingColumn]].to_string(index=False))

    bCalculateRatios, nWorkingColumn, my_columns = GetColumns()

for i in my_columns:
    # if we are calculating a ratio, we need to create a new column to calcuate the ratio before charting
    # otherwise, we literally chart the attribute
    # eg. X-Axis = Age
    #     Y-Axis = A ratio ... OR ... an attribute
    if (bCalculateRatios):
        # we only calcualte every second ratio.  If it's the second iteration, skip
        if num % 2 == 1:
            continue
        ##### continue here

    attr = SelectNextColumnFromFile(df, i)
    ...
    if (attr != ""):
        ...
        while (sexes_count > 0):
            polynomial_order = 1
            bAlwaysUseRecommended, bNeverUseRecommended, extra_filter, mid_point = SetLowHighFilterValues(bAlwaysUseRecommended, bNeverUseRecommended)
