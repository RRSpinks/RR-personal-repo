...
def GetColumns():
    ...
    print()
    uin1 = input("Which column numbers to use for x-axis, y-axis of the graph,\neg. 10, 22, 35, 41?\neg. 10, 22, 35, 41 would chart 10 vs 22, then 35 vs 41: ")
...
def BuildFileList():
    ...
    if (len(filelistcsv) > 0) or (len(filelistxlsx) > 0):
        ...
        if (i >= '1') and (i <= '9'):
            ...
            filename = os.path.basename(filelistcsv[int(i)-1])
            ...
            filename = os.path.basename(filelistxlsx[ord(i)-97])
...
def GetSexesToChart():
    ...
    while (len(chart_sexes) == 0):
        uin = input("Chart (m)ale, (f)emale, (b)oth sexes, (c)ombined sexes, (a)ll charts : ").lower()
...
# will return a query string for a generic field (STR_GENERIC_FIELD)
...
# note: this connersion is done outside of this function
def GetFilterRange(bFilterAlways, bFilterNever, strFieldName):
    strFilter = ""

    uin = input(f"Enter filter range [a|n](min, max) for {strFieldName}\neg. a1.5, 2.0\na = always\nn=never ").lower()
    if (uin != ""):
        if (uin[0] == 'a'):
            bFilterAlways = True
            uin = uin.replace('a', '')
...
# determine the number of sexes (and which sex types) to chart
